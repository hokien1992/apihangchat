...
(function($) {
    ...
    $(".title_box_user_left_all").click(function(){
      $(this).parent().find('.box_user_option').toggleClass('open_title_box_user_left_all');
    })
    ...
    $(window).bind('scroll', function () {
      if($(window).width()<768){
        ...
        if ($(window).scrollTop() > 100) {
          console.log('fix menu > 100');
            $('.header-nav.mid-header').addClass('fixed');
        } else {
          console.log('fix menu < 100');
            $('.header-nav.mid-header').removeClass('fixed');
        }
      }
